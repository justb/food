...
var insertOrders = function(totalprice,createdate,userid, callback) {
    pool.getConnection(function(err, connection) {
        if (err) {
            console.log(err);
            return callback(err);
        ...
        connection.query('insert into t_ordertotal set totalprice=?,ordertime=?,userid=?', [totalprice,createdate,userid], function(err, results) {
            if (err) {
                return callback(err);
            }
            connection.release();
