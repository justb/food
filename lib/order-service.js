...
var getOrdersById = function(id, callback) {
    pool.getConnection(function(err, connection) {
        ...
        //条件参数id
        connection.query('SELECT * FROM orders where id=?', id, function(err, results) {
            if (err) {
                return callback(err);
            }
            connection.release();
...
var insertOrders = function(totalprice,createdate, callback) {
    pool.getConnection(function(err, connection) {
        if (err) {
            console.log(err);
            return callback(err);
        ...
        connection.query('insert into t_ordertotal set totalprice=?,ordertime=?,userid=?', [totalprice,createdate,'test_user'], function(err, results) {
            if (err) {
                return callback(err);
            }
            connection.release();
...
var insertMenus = function(totalprice,createdate, callback) {
    pool.getConnection(function(err, connection) {
        if (err) {
            return callback(err);
        }
        //两个参数
        connection.query('insert into t_order set menuid=?,orderno=?', [totalprice,createdate], function(err, results) {
            if (err) {
                return callback(err);
            }
            connection.release();
